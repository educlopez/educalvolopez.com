...
const ResultItem = React.forwardRef(({ action, active }, ref) => {
  return (
    <div
      ref={ref}
      className="flex items-center justify-between px-4 py-2 m-0 text-gray-700 cursor-pointer dark:text-white"
    >
        ...
        ? (
          <div aria-hidden className="grid grid-flow-col gap-3">
            {action.shortcut.map((shortcut) => (
              <kbd
                key={shortcut}
                className="p-2 text-gray-700 uppercase bg-white rounded dark:text-white dark:bg-black border-1 bg-opacity-70"
              >
                {shortcut}
              </kbd>
            ))}
